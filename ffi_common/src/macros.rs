...
simplify memory management.
            """]
            pub unsafe extern "C" fn [<ffi_array_ $t _init>](
                ptr: *const $t,
                len: isize,
            ) -> [<FFIArray $t>] {
                if ptr.is_null() {
...
We're assuming that the memory in the `array` you give us was allocated by Rust (either internally
or through the provided FFI constructor `ffi_array_*_init`). Don't call this with an object created
on the other side of the FFI boundary; that is undefined behavior.
...
you're finished with it on the consumer side. Otherwise you will leak memory.
            """]
            #[no_mangle]
            pub extern "C" fn [<option_ $t _init>](has_value: bool, value: $t) -> *const $t {
                 if has_value {
...
Ownership gets a bit complicated here because there are two different things to think about.

First, this struct needs to be brought back into Rust ownership to be safely deallocated by the Rust
allocator, just like any other type we pass over the FFI. When you're done with this `FFIArray*`
type, call `ffi_array_*_free` to let Rust reclaim it.

Second, the structs that the elements of this `FFIArray*` point to need to be brought back into Rust
ownership. This **does not** happen automatically as part of calling `ffi_array_*_free`. The
consumer is expected to take ownership of that data and free each element when it's done. The types
generated by `ffi_consumer` will automatically take care of that.

In summary, interacting with this struct from the FFI consumer's perspective looks like this:
1. Call a Rust FFI method that returns an `FFIArray*`.
2. Iterate through its elements (which are pointers) and retain them for as long as necessary (until
    you've copied the data, )
3. When the data is no longer needed, free each element by calling the appropriate `free_*` method.
            """]
            #[repr(C)]
            #[allow(missing_copy_implementations)]
            #[derive(Clone, Debug)]
...
Note that this reclaims ownership **of the array structure**, not of its elements (see the
`FFIArray*` type's docs for more details).
...
The elements of `array` are pointers, and the memory to which they point is **not** reclaimed by
this method. You are still responsible for calling the appropriate `free` method on each element of
`array` when you no longer need it.
