...
use ffi_common::{self, string::FFIArrayString, datetime::FFIArrayTimeStamp, ffi_string, try_or_set_error};
...
fn test_struct_ffi() {
    ...
    unsafe {
        ...
        let input_date_vec = Some(vec![NaiveDateTime::from_timestamp(1599868112, 0), NaiveDateTime::from_timestamp(653010512, 0)]);

        // Convert to the FFI types that the FFI consumer will be passing in.
        let ffi_string = ffi_string!(input_text);
        let ffi_i32_array = ffi_common::ffi_array_i32_init(input_i32_vec.as_ptr(), input_i32_vec.len().try_into().unwrap());
        let ffi_string_array = FFIArrayString::from(&*input_string_vec);
        let ffi_uuid_array = FFIArrayString::from(&*input_uuid_vec);
        let ffi_variant_array = test_enum_ffi::ffi_array_test_enum_init(input_variant_vec.as_ptr(), input_variant_vec.len().try_into().unwrap());
        ...
        assert_eq!(input_text, CStr::from_ptr(get_test_struct_string(test_struct)).to_string_lossy());
        assert_eq!(input_i32_vec, Vec::from(get_test_struct_i32_collection(test_struct)));
        assert_eq!(variant, get_test_struct_enum_variant(test_struct));
        assert_eq!(double, get_test_struct_f64_thing(test_struct));
        assert_eq!(input_string_vec, Vec::<String>::from(get_test_struct_collection_of_strings(test_struct)));
        assert_eq!(input_uuid_vec, Vec::from(get_test_struct_collection_of_ids(test_struct)));
        assert_eq!(input_variant_vec, Vec::from(get_test_struct_collection_of_variants(test_struct)));
        assert_eq!(input_date_vec, Some(Vec::from(get_optional_test_struct_collection_of_dates(test_struct))));
